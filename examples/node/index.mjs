...
const said = process.argv.slice(2).join(' ') || 'أبي برجر دجاج وبطاطس، لا لا خله لحم'
...
if (!agentId) {
  ...
  const audio = await voho('/v1/speech', { text: 'أهلاً بك في مطعم تشاركو، معك ليلى. تحب تطلب إيش؟', voice: 'layla', model: 'sada-1', format: 'mp3' }, true)
...
const out = await voho(`/v1/agents/${agentId}/reply`, {
  text: said,
  variables: { company: process.env.VOHO_COMPANY ?? 'مطعم تشاركو' },
})
